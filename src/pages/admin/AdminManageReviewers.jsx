...
import {
  Users, CheckCircle, XCircle, Clock, Eye, Award,
  Briefcase, Mail, ExternalLink, Search, Filter
} from "lucide-react";
...
export default function AdminManageReviewers() {
  ...
  const handleReject = async () => {
    ...
    setActionLoading(true);
    try {
      await rejectReviewer(selectedRequest.userId, rejectReason);
      await loadRequests();
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500 mb-2">
                      ...
                      <span className="flex items-center gap-1">
                        <Briefcase className="w-3.5 h-3.5" /> {req.designation} — {req.institution}
                      </span>
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {req.expertise?.slice(0, 5).map(e => (
                        <span key={e} className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs rounded-full">
                          {e}
                        </span>
                      ))}
                      {req.expertise?.length > 5 && (
                        <span className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-full">
                          +{req.expertise.length - 5} more
                        </span>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 text-white p-6 rounded-t-2xl">
              <div className="flex justify-between items-start">
                <div>
                  <h2 className="text-xl font-bold">{selectedRequest.fullName}</h2>
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><p className="text-gray-500">Email</p><p className="font-medium">{selectedRequest.email}</p></div>
                <div><p className="text-gray-500">Status</p>{statusBadge(selectedRequest.status)}</div>
                <div><p className="text-gray-500">Submitted</p><p className="font-medium">{selectedRequest.submittedAt?.toDate?.().toLocaleDateString()}</p></div>
                {selectedRequest.orcid && <div><p className="text-gray-500">ORCID</p><p className="font-medium">{selectedRequest.orcid}</p></div>}
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRequest.expertise?.map(e => (
                    <span key={e} className="px-3 py-1 bg-indigo-50 text-indigo-700 text-sm rounded-full">{e}</span>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">{selectedRequest.experience}</p>
              </div>
      ...
      {showRejectModal && selectedRequest && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl p-6">
            ...
            <p className="text-gray-600 text-sm mb-4">Rejecting <strong>{selectedRequest.fullName}</strong>. Please provide a reason:</p>
            <textarea
              rows={4}
              value={rejectReason}
