...
import {
  sendReviewerApprovedEmail,
  sendReviewerRejectedEmail,
...
export default function AdminManageReviewers() {
  ...
  const handleApprove = async (req) => {
    ...
    try {
      ...
      // ✅ Send approval email
      try {
        await sendReviewerApprovedEmail({
          email: req.users?.email,
          name:
            `${req.users?.given_name || ""} ${req.users?.family_name || ""}`.trim() ||
            "Reviewer",
        });
      } catch (emailError) {
        console.warn(
          "⚠️ Approval email failed (non-blocking):",
          emailError.message,
        );
      }
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      try {
        await sendReviewerRejectedEmail({
          email: selectedRequest.users?.email,
          name:
            `${selectedRequest.users?.given_name || ""} ${selectedRequest.users?.family_name || ""}`.trim() ||
            "Applicant",
          reason: rejectReason,
        });
      } catch (emailError) {
        console.warn(
          "⚠️ Rejection email failed (non-blocking):",
          emailError.message,
        );
      }
  ...
  const filtered = requests.filter((r) => {
    ...
    const name =
      `${r.users?.given_name || ""} ${r.users?.family_name || ""}`.toLowerCase();
    const matchesSearch =
      !search ||
  ...
  const statusBadge = (status) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${styles[status]}`}
      >
    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-bold flex items-center gap-3">
          <Users className="w-8 h-8 text-indigo-600" /> Manage Reviewer
          Applications
        </h1>
        <p className="text-gray-600 mt-1">
          Review applications and approve or reject reviewer requests.
        </p>
      </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500 mb-2">
                        <span className="flex items-center gap-1">
                          <Mail className="w-3.5 h-3.5" /> {req.users?.email}
                        </span>
                        <span className="flex items-center gap-1">
                          <Briefcase className="w-3.5 h-3.5" />{" "}
                          {req.designation} — {req.institution}
                        </span>
                      </div>
                    </div>
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 text-white p-6 rounded-t-2xl">
              <div className="flex justify-between items-start">
                ...
                <button
                  ...
                  className="p-1 hover:bg-white/20 rounded-lg transition text-xl font-bold"
                >
                  ✕
                </button>
              </div>
