...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div
      className={`fixed bottom-6 right-6 z-50 flex items-center gap-3 px-5 py-4 rounded-xl text-white shadow-2xl animate-slide-up ${
...
export default function AdminAssignReviewers() {
  ...
  const loadData = async () => {
    try {
      ...
      // Only show papers that are submitted or under review
      setPapers(
        allPapers.filter((p) =>
  ...
  const handleAssign = async () => {
    ...
    try {
      const reviewer = reviewers.find(
        (r) => r.users?.email === selectedReviewer,
      );
      const reviewerName =
        `${reviewer.users?.given_name || ""} ${reviewer.users?.family_name || ""}`.trim() ||
  ...
  // ✅ Get the paper author's email — checks paper_authors first, then users join
  const getPaperAuthorEmail = (paper) => {
    const corresponding = (paper.paper_authors || []).find(
      (a) => a.is_corresponding,
    );
    if (corresponding?.email) return corresponding.email.toLowerCase();
    if (paper.paper_authors?.[0]?.email)
      return paper.paper_authors[0].email.toLowerCase();
    if (paper.users?.email) return paper.users.email.toLowerCase();
    return "";
  };

  // ✅ Exclude the paper's own author from the reviewer dropdown
  const getEligibleReviewers = (paper) => {
    if (!paper) return reviewers;
    const authorEmail = getPaperAuthorEmail(paper);
    return reviewers.filter(
      (r) => (r.users?.email || "").toLowerCase() !== authorEmail,
    );
  };
    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Assign papers to registered reviewers for peer review
        </p>
      </div>
        ...
        <div className="divide-y">
          {filteredPapers.length === 0 ? (
            ...
            filteredPapers.map((paper) => {
              ...
              const completedReviews = paperAssignments.filter(
                (a) => a.status === "completed",
              ).length;
              const corresponding =
                (paper.paper_authors || []).find((a) => a.is_corresponding) ||
                paper.paper_authors?.[0];
              const authorName =
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        ...
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_COLORS[paper.status] || "bg-gray-100 text-gray-600"}`}
                        >
                      ...
                      {paperAssignments.length > 0 && (
                        <div>
                          <p className="text-sm font-medium text-gray-700 mb-2">
                            Reviewers ({completedReviews}/
                            {paperAssignments.length} submitted):
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {paperAssignments.map((a) => {
                              const rName =
                                `${a.users?.given_name || ""} ${a.users?.family_name || ""}`.trim() ||
                                a.users?.email ||
                                "Reviewer";
                              return (
                                <span
      ...
      {showAssignModal &&
        selectedPaper &&
        (() => {
          const eligible = getEligibleReviewers(selectedPaper);
          const authorExcluded = reviewers.length !== eligible.length;
          return (
                ...
                <div className="p-6 space-y-5">
                  {selectedPaper.file_url && (
                    <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-lg flex items-center justify-between">
                      <span className="text-sm text-indigo-700 font-medium">
                        View manuscript before assigning
                      </span>
                      <a
                  ...
                  <div>
                    ...
                    {eligible.length === 0 ? (
                      <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg text-amber-700 text-sm">
                        No approved reviewers available for this paper.
                      </div>
                    ) : (
                      <select
                      ...
                      >
                        ...
                        {eligible.map((r) => {
                          const name =
                            `${r.users?.given_name || ""} ${r.users?.family_name || ""}`.trim() ||
                            r.users?.email;
                          return (
                    ...
                    {/* ✅ Show warning if the author was excluded */}
                    {authorExcluded && (
                      <p className="text-xs text-amber-600 mt-2 flex items-center gap-1">
                        <AlertCircle className="w-3.5 h-3.5" />
                        The paper's author has been excluded from the reviewer
                        list.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button
                      ...
                      disabled={assigning || eligible.length === 0}
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-green-200 text-xs mt-1">
                    {selectedPaperReviews.length} review(s) received
                  </p>
                </div>
                <button
            ...
            <div className="p-6 space-y-6">
              {selectedPaperReviews.length === 0 ? (
                ...
                selectedPaperReviews.map((review, index) => {
                  const reviewerName =
                    `${review.users?.given_name || ""} ${review.users?.family_name || ""}`.trim() ||
                    review.users?.email ||
                    "Reviewer";
                  return (
                    <div
                    ...
                    >
                      ...
                      <div className="grid grid-cols-5 gap-3 mb-4">
                        {[
                          {
                            label: "Originality",
                            value: review.originality_rating,
                          },
                          {
                            label: "Methodology",
                            value: review.methodology_rating,
                          },
                          { label: "Clarity", value: review.clarity_rating },
                          {
                            label: "Significance",
                            value: review.significance_rating,
                          },
